...
<div class="col-xs-6 col-sm-3 sidebar-offcanvas">
    ...
    <form role="form">
        <div class="form-group">
            ...
            <?php echo CHtml::dropDownList('id', array(), CHtml::listData(Locations::model()->findAll(),'id','name'),
    ...
    <?php if (!empty($places['results'])): ?>
		<?php Yii::app()->clientScript->registerScript('loadMap', "Main.loadMap({$location->lat}, {$location->long});"); ?>
		<?php 
			// Center the map with the origin marker
			$lat = $location->lat;
			$long = $location->long;
			$name = $location->name;
			Yii::app()->clientScript->registerScript('origin', "
					Main.addMarker(
		...
		<ul>
			<?php foreach ($places['results'] as $place): ?>
				...
				<?php
					...
					$name = $place['name'];
					$icon = $place['icon'];
					Yii::app()->clientScript->registerScript('loadMarker-' . $place['id'], "
						Main.addMarker(
							Main.createLocation('{$lat}', '{$long}'), 
							\"{$name}\"
						);
	...
	<?php else: ?>
		<?php Yii::app()->clientScript->registerScript('loadMap', "Main.loadMap();"); ?>
	<?php endif; ?>
</div>